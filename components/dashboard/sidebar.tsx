  ...
  return (
    <aside className={`fixed top-0 left-0 z-40 ${sidebarWidth} h-full border-r border-slate-200 bg-white hidden md:block transition-all duration-300 ease-in-out`}>
      <div className="flex h-full flex-col">
        {/* Header */}
        <header className="flex h-14 lg:h-[60px] items-center border-b border-slate-200 px-3">
          <div className="flex items-center justify-between w-full">
            {isCollapsed ? (
              ...
              <Link href="/" className="flex items-center gap-2 font-semibold text-slate-900">
                ...
                <span className="transition-opacity duration-300">Dona Theresa</span>
              </Link>
            )}
            ...
            <Button
              ...
              className={`h-8 w-8 p-0 hover:bg-slate-100 ${isCollapsed ? 'ml-0' : 'ml-2'}`}
            >
              {isCollapsed ? (
